...
def test_block_shapes(field):
    n = field.n_modes
    st = MissileState(np.array([X0, 0.05, -0.03, 0.1, -0.2, 0.15]), np.zeros(6))
...
def test_rigid_canister_limit(field):
    ...
    st = MissileState(np.array([X0, 0.04, -0.02, 0.2, -0.1, 0.05]), y)
    kin = MissileKinematics(field).evaluate(st, np.zeros(n), np.zeros(n))
...
def test_omega_identity(field):
    ...
    st = MissileState(np.array([X0, 0.05, -0.04, 0.1, -0.2, 0.3]), y)
...
def test_translational_velocity_finite_difference(field):
    ...
    st = MissileState(x0, y0)

    kin = MissileKinematics(field).evaluate(st, p0, p_dot)
