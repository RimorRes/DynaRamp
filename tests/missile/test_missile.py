...
from dynaramp.missile import Missile, Slider, MissileState
...
# Missile
# --------------------------------------------------------------------------- #
def test_missile_coercion_and_defaults():
    m = Missile(mass=100, inertia_com=np.diag([1.0, 2.0, 3.0]))
...
def test_inertia_o1_equals_com_when_no_offset():
    ...
    m = Missile(mass=5.0, inertia_com=inertia, com_o1=(0, 0, 0))
...
def test_inertia_o1_parallel_axis():
    ...
    m = Missile(mass=2.0, inertia_com=np.identity(3), com_o1=(1.0, 0.0, 0.0))
...
def test_inertia_o1_matches_manual_formula():
    ...
    mass = 7.5
    m = Missile(mass=mass, inertia_com=inertia, com_o1=r)
...
def test_slider_coercion():
    ...
    m = Missile(mass=1.0, inertia_com=np.identity(3), sliders=[s])
