...
from dynaramp.missile import Missile, MissileEOM, MissileKinematicState
...
def test_block_shapes():
    ...
    missile = Missile(mass=120.0, inertia_com=_spd_inertia(2), com_o1=(0.5, 0.1, -0.2))
...
def test_blocks_match_equations_cross_product():
    ...
    missile = Missile(mass=87.0, inertia_com=_spd_inertia(4), com_o1=(0.4, -0.3, 0.2))
    eom = MissileEOM.assemble(kin, missile)

    m = missile.mass
    r = kin.a_ib @ missile.com_o1
    i_o1 = kin.a_ib @ missile.inertia_o1 @ kin.a_ib.T
    w = kin.omega_ib
...
def test_translational_eom_reduces_to_newton_when_com_at_o1():
    ...
    missile = Missile(mass=42.0, inertia_com=_spd_inertia(6), com_o1=(0, 0, 0))
    eom = MissileEOM.assemble(kin, missile)
    ...
    assert np.allclose(lhs, missile.mass * a_o1)
...
def test_rotational_eom_reduces_to_euler_when_com_at_o1():
    ...
    missile = Missile(mass=42.0, inertia_com=inertia, com_o1=(0, 0, 0))
    eom = MissileEOM.assemble(kin, missile)
